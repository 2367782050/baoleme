...
import { listArticleJobs } from "@/lib/services/admin.service";
...
export async function GET(req: NextRequest) {
  try { await requireAdmin(); const p = req.nextUrl.searchParams;
    const { items, total } = await listArticleJobs({ status: p.get("status") ?? undefined, page: parseInt(p.get("page") ?? "1"), pageSize: parseInt(p.get("pageSize") ?? "20") });
