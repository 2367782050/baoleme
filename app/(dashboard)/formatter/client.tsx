...
export function FormatterClient() {
  ...
  useEffect(() => {
    ...
    async function load(id: string) {
      ...
      try {
        const r = await fetch(`/api/articles/${id}`); const b = await r.json();
        if (cancelled) return;
        if (!b.success) { setError("文章加载失败"); return; }
        const a = b.data;
        setTitle(a.title ?? "");
        setMarkdown(a.markdownContent ?? "");
        if (a.formatterConfig) { const fc = a.formatterConfig as Partial<FormatterConfig>; setConfig({ ...DEFAULT_CONFIG, ...fc }); }
  ...
  async function handleRender() {
    setError("");
    try {
      const r = await fetch("/api/formatter/render", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ markdown, ...config }) });
