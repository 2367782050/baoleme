...
describe("Article generation service", () => {
  ...
  it("executes → completed with markdown", async () => { await resetDailyQuota(userId); const { article, job } = await createArticleGenerationJob(userId, { title: "测试文章生成", promptId }); await executeArticleGenerationJob(job.id); const j = await getArticleGenerationJob(job.id, userId); expect(j!.status).toBe("completed"); const a = await prisma.article.findUnique({ where: { id: article.id } }); expect(a!.status).toBe("completed"); expect(a!.markdownContent).toContain("#"); });

  it("mock review passes without rewrite", async () => { await resetDailyQuota(userId); const { article, job } = await createArticleGenerationJob(userId, { title: "通过审核", promptId }); await executeArticleGenerationJob(job.id); const a = await prisma.article.findUnique({ where: { id: article.id } }); expect(a!.markdownContent).not.toContain("【精修】"); });

  it("mock review forces rewrite", async () => { mockAIProvider.setForceRewrite(true); await resetDailyQuota(userId); const { article, job } = await createArticleGenerationJob(userId, { title: "需要重写", promptId }); await executeArticleGenerationJob(job.id); const a = await prisma.article.findUnique({ where: { id: article.id } }); expect(a!.markdownContent).toContain("【精修】"); });

  it("mock failure → failed with errorMessage", async () => { mockAIProvider.setFailNext(true); await resetDailyQuota(userId); const { article, job } = await createArticleGenerationJob(userId, { title: "会失败", promptId }); try { await executeArticleGenerationJob(job.id); } catch {} const j = await getArticleGenerationJob(job.id, userId); expect(j!.status).toBe("failed"); expect(j!.errorMessage).toContain("Mock AI failure"); const a = await prisma.article.findUnique({ where: { id: article.id } }); expect(a!.status).toBe("failed"); });

  it("retry failed→pending→completed", async () => { mockAIProvider.setFailNext(true); await resetDailyQuota(userId); const { article, job } = await createArticleGenerationJob(userId, { title: "重试测试", promptId }); try { await executeArticleGenerationJob(job.id); } catch {} const r = await retryArticleGenerationJob(job.id, userId); expect(r.status).toBe("pending"); await resetDailyQuota(userId); await executeArticleGenerationJob(r.id); const j2 = await getArticleGenerationJob(r.id, userId); expect(j2!.status).toBe("completed"); const a = await prisma.article.findUnique({ where: { id: article.id } }); expect(a!.status).toBe("completed"); });

  it("deducts article_generate quota on success", async () => { await resetDailyQuota(userId); const { job } = await createArticleGenerationJob(userId, { title: "配额测试", promptId }); await executeArticleGenerationJob(job.id); const after = await prisma.quotaUsage.findFirst({ where: { userId, capability: "article_generate" } }); expect(after!.used).toBeGreaterThanOrEqual(1); });
