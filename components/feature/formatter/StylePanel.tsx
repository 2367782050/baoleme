    ...
    <div className="space-y-4 text-sm">
      ...
      <div>
        ...
        <div className="flex gap-2">
          <div className="w-9 h-9 shrink-0 rounded-lg border border-white/30 overflow-hidden">
            <input type="color" value={config.themeColor} onChange={e => set("themeColor", e.target.value)} className="w-full h-full border-0 p-0 cursor-pointer" />
          </div>
          <input type="text" value={config.themeColor} onChange={e => set("themeColor", e.target.value)} className="min-w-0 flex-1 glass-input text-sm font-mono !py-1.5" /></div>
      </div>
      <div>
        ...
        <select value={config.fontFamily} onChange={e => set("fontFamily", e.target.value)} className="w-full glass-input text-sm !py-1.5">
          {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}</select>
      </div>
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">字号 ({config.fontSize}px)</label>
        <input type="range" min={12} max={28} value={config.fontSize} onChange={e => set("fontSize", Number(e.target.value))} className="w-full glass-range" />
      </div>
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">行距 ({config.lineHeight})</label>
        <input type="range" min={1.2} max={4} step={0.1} value={config.lineHeight} onChange={e => set("lineHeight", Number(e.target.value))} className="w-full glass-range" />
      </div>
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">段距 ({config.paragraphSpacing}px)</label>
        <input type="range" min={0} max={60} value={config.paragraphSpacing} onChange={e => set("paragraphSpacing", Number(e.target.value))} className="w-full glass-range" />
      </div>
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">两侧边距 ({config.sidePadding}px)</label>
        <input type="range" min={0} max={60} value={config.sidePadding} onChange={e => set("sidePadding", Number(e.target.value))} className="w-full glass-range" />
      </div>
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">图片圆角 ({config.imageRounded}px)</label>
        <input type="range" min={0} max={30} value={config.imageRounded} onChange={e => set("imageRounded", Number(e.target.value))} className="w-full glass-range" />
      </div>
    </div>
  );
}
