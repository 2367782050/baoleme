...
export function StylePanel({ config, onChange }: { config: FormatterConfig; onChange: (c: FormatterConfig) => void }) {
  ...
  return (
    <div className="space-y-4 text-sm">
      <h3 className="font-semibold text-zinc-900">样式设置</h3>
      ...
      <div>
        ...
        <select value={config.style} onChange={e => onChange({ ...DEFAULT_CONFIG, ...getTemplateConfig(e.target.value), style: e.target.value as FormatterConfig["style"] })}
          ...
          {STYLE_TEMPLATES.map(t => <option key={t.key} value={t.key}>{t.name}</option>)}
        </select>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input type="color" value={config.themeColor} onChange={e => set("themeColor", e.target.value)} className="w-8 h-8 rounded border border-zinc-300 cursor-pointer" />
          <input type="text" value={config.themeColor} onChange={e => set("themeColor", e.target.value)} className="flex-1 rounded border border-zinc-300 px-2 py-1 text-sm font-mono" />
        </div>
      </div>
      ...
      <div>
        ...
        <select value={config.fontFamily} onChange={e => set("fontFamily", e.target.value)} className="w-full rounded border border-zinc-300 px-2 py-1 text-sm">
      ...
      <div>
        ...
        <input type="range" min={12} max={28} value={config.fontSize} onChange={e => set("fontSize", Number(e.target.value))} className="w-full" />
      </div>

      {/* Line height */}
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">行距 ({config.lineHeight})</label>
        <input type="range" min={1.2} max={4} step={0.1} value={config.lineHeight} onChange={e => set("lineHeight", Number(e.target.value))} className="w-full" />
      </div>

      {/* Paragraph spacing */}
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">段距 ({config.paragraphSpacing}px)</label>
        <input type="range" min={0} max={60} value={config.paragraphSpacing} onChange={e => set("paragraphSpacing", Number(e.target.value))} className="w-full" />
      </div>

      {/* Side padding */}
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">两侧边距 ({config.sidePadding}px)</label>
        <input type="range" min={0} max={60} value={config.sidePadding} onChange={e => set("sidePadding", Number(e.target.value))} className="w-full" />
      </div>

      {/* Image rounded */}
      <div>
        <label className="block text-xs font-medium text-zinc-500 mb-1">图片圆角 ({config.imageRounded}px)</label>
        <input type="range" min={0} max={30} value={config.imageRounded} onChange={e => set("imageRounded", Number(e.target.value))} className="w-full" />
      </div>
    </div>
  );
}
